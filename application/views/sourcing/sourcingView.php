				...
				<div class="addproducts-con">
					...
					<div class="sourcing-addpro-main" id="sourcing1">					
						...
						<div class="addpro-buttons">
							<a class="add-pro-cancel" href="<?php current_url();?>">Cancel</a>
							<a class="add-pro-submit" id="submitsource" href='javascript:void(0)'>Submit</a>
						</div>
					</div>
					<form method="post" id = "formid" action="create-sourcing-product"  enctype="multipart/form-data">
							...
							<div class="addpro-detail indiv-prdct">
								<ul class="individual-product">				   
									...
									<li class="images-list">
										...
										<div class="input-group">
											<input  type="file" class="upload-inp" id="gallery-photo-add" name="sourcing_image[]" multiple>
											<img src="<?=base_url('assets/images')?>/iconplus.png">
											<?php echo form_error('sourcing_image'); ?> 
											<div class="review-image gallery"></div>
											<span id="image-counter">You can upload up to 5 images.</span>   
										</div>		
									</li>
								...
								<div class="addpro-buttons">
									<a class="add-pro-cancel" href="echo <?php current_url();?>">Cancel</a>
	...
	<script>
		 $(document).ready(function(){
			...
			for(var count = 0; count<files.length; count++)  
				{
				  form_data.append("sourcing_image[]", files[count]);
				   $.ajax({
					url:'<?=base_url()?>Sourcing/upload_sourcing_image',
					method:"POST",
					data: form_data,
					contentType: false,
					cache: false,
					...
					{
						$("#formid").append("<input type = 'hidden' name='imges[]' value='"+data+"'></input>");						
					}
				   });
				}
			});			
		...
		$(document).ready(function(){
			/************ Image preview before uploding images *************/
		...
		$(document).ready(function() {
			...
			function paginationData(data) {
				...
				for(emp in data){ 
					...
					empRow +="<td><a href='https://" + store_url +"/products/"+ data[emp].handle +"' target='_blank'> "+store_url+"/products/"+ data[emp].handle+"</td><td>$ "+ data[emp].variants[0].price +"</td>";
			...
			$("#selectall-checkbox").change(function(){
				if($(this).prop("checked")){
					$('.sourcing_image_url').each(function(i){
					   $(this).addClass('active_source_img').prop('checked',true);
					});
				}else{
						$('.sourcing_image_url').each(function(i){
						$(this).removeClass('active_source_img').prop('checked',false);
					});
				}
			});
