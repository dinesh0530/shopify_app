	...
	<form id = "form_id">
	  ...
	  ?>       
		...
		<div class="errormsg error" id="error"></div>	
			...
			<ul role="tablist" id = "remove_products" class="ui-tabs-nav ui-corner-all ui-helper-reset ui-helper-clearfix ui-widget-header">
			   <li><a href-"#"><span class="for-checkbox">
				  <input class="import-products variant-selector" id = "mylist_chkbox" name="mylist_chkbox[]" type="checkbox" value="<?php echo $product['product_id'];?>"></span></a>
			   </li>
		...
		$( function(){ 
		   $( "#tabs<?php echo $product['product_id']; ?>" ).tabs(); 
			...
			<div class="remove added-remove-links">
				...
				<a id="remove_<?php echo $product['product_id'];?>" href="javaScript:void(0);" class="remove-product ">Remove</a>
			</div>    
				...
				<div class="list-variants-body">
					<table class="table-condensed variants-table" id="bulk-suggested-price<?=$product['id'];?>">
						...
						if(!empty($product['product_mylist_variants'])){
						  ...
						  $compared_at_price = $product['default_variants_price'];
						}
						else{
						  $variant_your_price = $variant['price'];
						  $compared_at_price = $product['default_variants_price'];
						}
	          	...
	          	echo "<ul class='image-selector__draggable'>";
					...
					$ext = pathinfo($filename, PATHINFO_EXTENSION);
					 ...
					 <li class="image-selector__thumbnail">
					   <?php  if(!empty($product['product_images_ids']) && ($product['user_id'] == $this->session->userdata('login')->id)){
							$product_images_ids = $product['product_images_ids'][0]['image_ids'];
							$product_images_ids = explode(",",$product_images_ids);								
							if (in_array($productImage['id'], $product_images_ids))
							{
							   $add_class = 'product-image--is-selected';
							}
							else
							{
							   $add_class ='';
						...
						?>
						 <div class="product-image <?=$add_class?> var-<?=$productImage['variant_id']?>" id="varient-img<?=$productImage['id']?>">							   
						   <?php }else{?>
						   <div class="product-image product-image--is-selected var-<?=$productImage['variant_id']?>" id="varient-img<?=$productImage['id']?>">	
						   <?php }?>
...
<div id="alert_products" class="common_popup delete_popup"  title="Warning " style="display:none;">
  <p><span class="ui-icon ui-icon-alert"></span>You can select maximum 3 products</p>
</div>
<div id="remove_products_popup" class="common_popup delete_popup"  title="Remove Products!" style="display:none;">
  <p><span class="ui-icon ui-icon-alert"></span>Are you want to remove all products?</p>
</div>
...
$(".remove-product").click(function (e) {	
  ...
  $("#bulk-delete-action").click(function (e) {
	var product_id = [];
	$(".My-List-Page-inner").each(function(){
	var id = $(this).find('#mylist_chkbox').val();
	product_id.push(id);
	});
	$("#remove_products_popup").dialog({
		  modal: true,
		  buttons: { 
			"Confirm": function() {
						$.ajax({
			...
			var modal = $('#remove_products_popup').dialog('isOpen');
		if(modal == true){
			$('.ui-dialog').addClass('delete_popup');
			$('.overlay').show();
		} 
		$('#remove_products_popup').on('dialogclose', function(event) {
			$('.ui-dialog').removeClass('delete_popup');
    	$('.overlay').hide();
 		});		
    });	
...
$('.import-products:checkbox').on("change", function(e){
		...
		if($(this).prop("checked")){
			var num = getSelectedCheckboxCount();
			if(num <= 3){
		...
		if(modal == true){
			$('.ui-dialog').addClass('delete_popup');
			var id = $('.ui-dialog').children().children().attr('class');
			  $('.'+id).html('<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Warning');
			$('.overlay').show();
		} 
		$('#alert_products').on('dialogclose', function(event) {
			$('.ui-dialog').removeClass('delete_popup');
    	$('.overlay').hide();
 		});	
	});	
...
$('#bulk-selector:checkbox').on("change", function(e){	
			...
			if(num <= 3){
				...
				$('.main-tab-div').each(function(i){
					...
					$('#import-button').removeAttr("disabled");
					$(".import-products:eq(0)").prop("checked", true);
					$(".import-products:eq(1)").prop("checked", true);
					$(".import-products:eq(2)").prop("checked", true);
					$('.selected-count').text('3 Products selected');
				});
				/* for (int i = 0; i < 3; i++) {
				...
				$("#alert_products").dialog({  
					  ...
					  modal:true,
				//	  title:'Please select only 3 products',
					  width:400,
					  buttons: {
						Ok: function() {
						  $( this ).dialog( "close" );
		...
		$('.bulk-variant-selector:checkbox').on("change", function(e){
			if($(this).prop("checked")){				
				...
				$('.product-image').addClass('product-image--is-selected');
			} 
			else {
				var table= $(e.target).closest('table');
				...
				$('.product-image').not(':first').removeClass('product-image--is-selected');
				$('.product-image:lt(2)').addClass('product-image--is-selected');
			}	
...
$('.single-variant-row input:checkbox').on("change", function(e){
			...
			var varid = $(this).parent().find('input.vid').val();			
			if($(this).prop("checked")){				
				$(this).closest("tr").addClass('row-enabled');
				$('.var-'+varid).addClass('product-image--is-selected');
			} else {
				    if(selectedCheckbox > 0){					 
						$(this).closest("tr").removeClass('row-enabled');
					    $('.var-'+varid).removeClass('product-image--is-selected');
					} 
					else {				
  ...
  $('#import-button').on("click", function(e){   
	...
	$.ajax({
	  ...
	  /*  $('.variants-save-links a').on("click", function(e){	
		...
		var productImages = [];
		var variants_img =  $(this).parents('.save-selected-data').find('div.image-selector li');
		$(variants_img).each(function () {		
            var image_id = $(this).find('.product-image').attr('id').match(/\d+/);
			var varimgId = "#"+$(this).find('.product-image').attr('id');
		
			if ($(varimgId).hasClass("product-image--is-selected")){
				var varint_img_id = image_id;
	        	productImages.push(varint_img_id);				
			}else{
				var varint_img_id = 0;	
				productImages.push(varint_img_id); 
			} 
		...
		numeric.push({'id':id,'title':val,'collection':collect ,'type':val_type ,'category':category ,'tags':tag.replace(/\s+/g, " ").trim(),
		'description':desc,'variants':variantAll,'images':productImages});	
		$.ajax({
				type: "POST",
				url: "<?php echo base_url('Mylist/save_Mylist_data');?>", 
				data: {product_data: numeric},
