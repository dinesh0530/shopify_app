	...
	var $divs = $("div.vendor-orders");
		$('#total-sale').on('change', function (){
			if($(this).val() == "low_price"){
				...
				$(".vendor-main-orders").html(numericallyOrderedDivs);
            }
		...
		$('.orders_id').click(function(event){
			if($('#vendor_DB_order').is(':checked')){
				$('#vendor_DB_order').prop('checked', false);
			}
		});	
});	
</script>
...
?>
						...
						<div class="dashboard-top-section">
							<div class="dashboard-total-sales">
								...
								<span class="total-sales">
									<select id="total-sale" name="total-sales">
								...
								<div class="order-page-table dashboard-table-left">
									<div class="order-page-head">
										<div class="order-page-heading checkbox">
											<input id="vendor_DB_order" type="checkbox">
										</div>
										...
										<div class="order-page-heading vendor-name">
											Vendor name
										</div>
									...
									<div class="order-page-body vendor-main-orders">
									<?php
									   foreach ($orders as $order) {
										...
										?>
										<div class="vendor-orders page_<?=$className?>" style="display:none;">
											<div class="order-page-row">
												<div class="order-page-col checkbox"><input class="check_vendor_DB_order orders_id" type="checkbox" data-id="<?=$order_id?>"></div>
												...
												<div class="order-page-col vendor-name">
													<?php echo $vendorname; ?>
												</div>
												...
												<div class="order-page-body">
													<?php foreach($line_items as $orderPro){
														...
														<div class="order-page-row ordered-product">
															...
															<div class="order-page-col product-item pro-sku"><?php 
															if(!empty($orderPro['sku'])){
																echo $orderPro['sku'];
                                                            }else{
																echo "____";
															}
	...
	<?php $paginateLink = ceil($allOrders/5);
...
jQuery(document).ready(function($){
	...
	<?php if($allOrders > 5){ ?>
		window.pagObj = $('#pagination').twbsPagination({
			...
			onPageClick: function (event, page) {
				$('.vendor-orders').css('display','none');
	...
	$("#vendor_DB_order").click(function(){
	   if(this.checked) { 
			$('.check_vendor_DB_order').each(function() { 
				this.checked = true;     
			});
		}else{
			$('.check_vendor_DB_order').each(function() {
